...
def no_preference(prob):
    """ This preference selects any unopened square. """
    return prob == np.nanmin(prob)


def edges(prob):
    """ This preference selects edges and default to `no_preference` if there are no good edge squares. """
...
def corners2(prob):
    """ This preference selects the squares that are just out of the corner, e.g. (1, 1) instead of (0, 0). """
    selection = np.zeros(prob.shape, dtype=bool)
...
def edges2(prob):
    """ This preference selects squares that are just off of the edge, e.g. (1, 5) instead of (0, 5). """
    selection = np.zeros(prob.shape, dtype=bool)
