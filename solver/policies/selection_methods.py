...
def nearest(preferred, prob):
    ...
    # Nearest only works if an opened square exists to be near, otherwise just default to random.
    if np.isnan(prob).any():
        # Keep looking further and further until a 'best' is found. We do this by dilating the mask over the known
...
def centered(preferred, prob):
    """ Select cells that are as much in the center as possible. """
    # Start at the center.
...
def inward_corner(preferred, prob):
    """ Select cells that are as much near a corner. """
    # Start at the center.
    search_mask = np.zeros(preferred.shape, dtype=bool)
