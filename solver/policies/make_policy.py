...
def make_policy(preferences=no_preference, selection_methods=nearest):
    """ Make a policy that selects a square with the lowest probability, based on the `preference` using the given
        selection methods. The workings are as follow: if an optimal square is in the preference area, generated by
        `preference_selectors`, it is selected. If multiple such squares exist, `selection_method` selects one. If no
        such square exists, the next selection method is tried, defaulting to random if no preference fits.
        ...
        :param preferences: A function or a list of functions that takes in the probability array and returns a mask for
                            the cells it would like to select. If a list is given, each preference method is used in
        ...
        :param selection_methods: A function or list of functions that takes the preference mask and selects from and
                                  ...
                                  selected in the end, a random one is chosen. Some examples can be found in
